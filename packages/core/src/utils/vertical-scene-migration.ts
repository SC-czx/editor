...
export function migrateVerticalSceneNodes(
  ...
  // Typical-floor links are a load-time invariant, not an edit-time one: a
  // master deleted in another client, a level moved between buildings, or a
  // hand-edited scene file can all leave an instance pointing at a master
  // that no longer exists (or at itself). Normalize the pair every load so
  // the editor never shows a level derived from nothing. Idempotent, and
  // lossless for well-formed scenes.
  for (const [id, node] of Object.entries(nodes)) {
    if (node?.type !== 'level') continue

    const record = node as Record<string, unknown>
    const isMaster = record.typicalMaster === true
    const storedMasterId =
      typeof record.typicalMasterId === 'string' && record.typicalMasterId.length > 0
        ? record.typicalMasterId
        : undefined
    const masterResolves =
      storedMasterId !== undefined &&
      storedMasterId !== id &&
      (nodes[storedMasterId] as AnyNode | undefined)?.type === 'level'

    // A level is either a master or an instance of one, never both — the
    // master flag is the half the user toggles, so it wins. A dangling or
    // self-referential back-reference is dropped rather than left to render
    // an instance with no source.
    const keepMasterFlag = isMaster
    const keepMasterId = !isMaster && masterResolves

    let next: Record<string, unknown> | null = null
    if (record.typicalMaster !== undefined && !keepMasterFlag) {
      next = { ...(next ?? record) }
      delete next.typicalMaster
    }
    if (record.typicalMasterId !== undefined && !keepMasterId) {
      next = { ...(next ?? record) }
      delete next.typicalMasterId
    }
    if (next) replaceNode(id, next)
  }
